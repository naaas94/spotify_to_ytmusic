...
def create_common_parser() -> ArgumentParser:
    """Create a parser with common arguments used by multiple commands."""
...
def create_playlist():
    ...
    parser.add_argument(
        "--privacy",
        default="PRIVATE",
        help="The privacy setting of created playlists (PRIVATE, PUBLIC, UNLISTED, default PRIVATE)",
    )
    parser.add_argument(
        "playlist_name",
        type=str,
...
def load_liked_albums():
    """
    Load the "Liked" albums from Spotify into YTMusic.
    Spotify stores liked albums separately from liked songs.
    """
...
def copy_playlist():
    ...
    parser.add_argument(
        "ytmusic_playlist_id",
        type=str,
        help="ID of the YTMusic playlist to copy to. If this argument starts with a '+', "
        "it is assumed to be the playlist title rather than playlist ID, and if a playlist "
        "of that name is not found, it will be created (without the +). "
        "Example: '+My Favorite Blues'. NOTE: The shell will require you to quote the name if it contains spaces.",
    )
    parser.add_argument(
        "--no-reverse-playlist",
        action="store_true",
